...
Namespace BizPad
	...
	Friend Class CodeMSIGeneratorHelper
		Inherits CodeMSIGenerator
		Implements IBarCodeGenerator
		Private Shared CheckSums As New Dictionary(Of String, MSICheckSum)()
		Shared Sub New()
		...
		Private Sub Initialize(ByVal switches As InstructionCollection) Implements IBarCodeGenerator.Initialize
			Dim c As String = If(switches.GetString("c"), String.Empty)

			Dim checksum As MSICheckSum = MSICheckSum.Modulo10
			If CheckSums.ContainsKey(c.ToLower()) Then
				checksum = CheckSums(c.ToLower())
			End If

			Me.MSICheckSum = checksum
		End Sub
