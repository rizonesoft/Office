...
namespace Rizonesoft.Verbum
{
    ...
    public class EmlDocumentExporter : IExporter<DocumentFormat, bool>
    {
        ...
        {
            documentModel.InternalAPI.SaveDocumentMhtContent(stream,
(EmlDocumentExporterOptions)options);
            return true;
        }
        #endregion
        ...
        public static void Register(IServiceProvider provider)
        {
            if (provider == null)
                return;
            IDocumentExportManagerService service =
provider.GetService(typeof(IDocumentExportManagerService)) as IDocumentExportManagerService;
            if (service != null)
